...
int fdct3d_forward(int N1, int N2, int N3, int nbscales, int nbdstz_coarse, int ac,
				   CpxNumTns& X, vector< vector<CpxNumTns> >& C)
{
  //  fft
  CpxNumTns T(X);
  ...
  CpxOffTns O;
  if(ac==1) {
	double L1 = 4.0*N1/3.0;  double L2 = 4.0*N2/3.0;  double L3 = 4.0*N3/3.0;
	int S1, S2, S3;  int F1, F2, F3;  double R1, R2, R3;	 fdct3d_rangecompute(L1, L2, L3, S1, S2, S3, F1, F2, F3, R1, R2, R3);
	IntOffVec t1(S1);
	for(int i=-F1; i<-F1+S1; i++)		if(     i<-N1/2) t1(i) = i+int(N1);		else if(i>(N1-1)/2) t1(i) = i-int(N1);		else t1(i) = i;
	IntOffVec t2(S2);
	for(int i=-F2; i<-F2+S2; i++)		if(     i<-N2/2) t2(i) = i+int(N2);		else if(i>(N2-1)/2) t2(i) = i-int(N2);		else t2(i) = i;
	IntOffVec t3(S3);
	for(int i=-F3; i<-F3+S3; i++)		if(     i<-N3/2) t3(i) = i+int(N3);		else if(i>(N3-1)/2) t3(i) = i-int(N3);		else t3(i) = i;
	O.resize(S1, S2, S3);
	for(int i=-F1; i<-F1+S1; i++)		for(int j=-F2; j<-F2+S2; j++)		  for(int k=-F3; k<-F3+S3; k++)			 O(i,j,k) = F(t1(i), t2(j), t3(k));
  } else {
	O = F;
  }
